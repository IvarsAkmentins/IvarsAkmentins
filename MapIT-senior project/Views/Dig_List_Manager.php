    ...
    <head>
        ...
        <script>
            $(document).ready(function() {
                ...
                var oTable = $('#dig_table').dataTable({
                    ...
                    "fnDrawCallback": function(oSettings){
                        $('.edit_dig').button({
                            ...
                            $('#digid_edit').val($(this).attr('id')); 
                            $('#digname_edit').val($(this).parent().prev().prev().prev().prev().prev().prev().prev().html()); 
                            $('#edit_form').submit();
                        });
                        ...
                        $('.delete_dig').button({
                            ...
                            $('#dialog').dialog({
                                ...
                                }).html('Are you sure you want to delete user: ' + $(this).parent().prev().prev().prev().prev().prev().prev().prev().html() + ' ?');
                        });
                    }
                });
                            ...
                            if (!data['digs']) {
                                ...
                                for(i = 0; i < data['digs'].length; i++){ 
                                    oTable.fnAddData([
                                        ...
                                        (data['digs'][i]['last_modified'] == '' || data['digs'][i]['last_modified'] <= 0) ? ' Never ' : convert_date(data['digs'][i]['last_modified'], 0),
                                        '<button class="edit_dig" id="' + data['digs'][i]['id'] + '">Edit Dig Site</button>' +
                                        '<button class="delete_dig" id="' + data['digs'][i]['id'] + '">Delete Dig Site</button>' 
                                    ]);
                                }
                            }
                        }
                ...
                function delete_dig_site(dig_id, row){
                    $.ajax({
                            url: '../Controllers/Map_Services.php',
                            type: 'POST',
                            dataType: 'json',
                            data: {
                                function_name: 'delete_dig_site_by_id',
                                id: dig_id
                            },
                            success: function(data) {                              
                                $('#dialog').dialog({
    ...
    <body>
        ...
        <div id="body" class="body_block_small">
            ...
            <div id="table" style="width: 95%; margin-left:20px; margin-top:20px">
                <table id="dig_table" class="display">
                    <thead>
                        <tr>
                            ...
                            <th width="85px" align="left">Initial Date</th>
                            <th width="85px" align="left">End Date</th>
                            <th width="90px" align="left">Last Modified</th>
                            <th width="75px"></th>
                        </tr>
                    </thead>
                </table>
            ...
            <div align="center" style="margin-top:15px; font-size: 14px">
                <button id="add_btn">Add New Dig Site</button>
            </div>
        </div>
        ...
        <form name="edit_form" id="edit_form" action="../Views/Dig_List_Edit.php" method="POST">
            <input type="hidden" name="digid_edit" id="digid_edit" value=-1>
            <input type="hidden" name="digname_edit" id="digname_edit" value=''>
        </form>        
    </body>
</html>
